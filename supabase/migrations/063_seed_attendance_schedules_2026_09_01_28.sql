-- supabase/seeds/attendance_schedules_2026_09_01_28.sql から自動生成（npm run db:seed-to-migration）
-- push すると GitHub Actions が本番へ適用します。
-- 松島 黄金 向けシフト登録（2026-09-01 〜 2026-09-28）
-- 「すず」追加。.5 は 30 分（例: 18.5 → 18:30）。
--
-- 前提:
...
shift_rows AS (
  ...
  FROM (
    VALUES
      ...
      ('2026-09-03'::date, '玉川',   '12:00:00'::time, '24:00:00'::time),
      -- 9/4（金）
      ...
      ('2026-09-10'::date, 'のあ',   '16:00:00'::time, '24:00:00'::time),
      ('2026-09-10'::date, 'ゆめ',   '15:00:00'::time, '24:00:00'::time),
      ('2026-09-10'::date, '玉川',   '15:00:00'::time, '24:00:00'::time),
      -- 9/11（金）
      ('2026-09-11'::date, 'ななみ', '16:00:00'::time, '24:00:00'::time),
      ('2026-09-11'::date, 'のあ',   '16:00:00'::time, '24:00:00'::time),
      ...
      ('2026-09-12'::date, 'すず',   '13:00:00'::time, '23:00:00'::time),
      ('2026-09-12'::date, 'みか',   '10:00:00'::time, '24:00:00'::time),
      -- 9/13（日）
      ('2026-09-13'::date, 'まる',   '10:00:00'::time, '24:00:00'::time),
      ('2026-09-13'::date, 'のあ',   '16:00:00'::time, '24:00:00'::time),
...
deleted AS (
  ...
  WHERE s.store_id = ts.store_id
    AND s.scheduled_date BETWEEN '2026-09-01' AND '2026-09-28'
    AND NOT EXISTS (
      SELECT 1
      FROM resolved r
