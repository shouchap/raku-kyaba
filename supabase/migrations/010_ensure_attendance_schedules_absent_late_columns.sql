-- attendance_schedules に is_absent / is_late / 理由カラムを保証する（009 未適用環境・Webhook 更新用）
-- IF NOT EXISTS のため、009 適用済みでも安全に再実行できる
...
-- 既存データ: response_status からフラグを復元（NULL の行は更新しない）
